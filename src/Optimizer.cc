...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    if(pbStopFlag) // 是否有这个Flag
        if(*pbStopFlag) // Flag里面的值是不是True
            return;
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        if(it!=CorrectedSim3.end())
        {
            ...
            vScw[nIDi] = it->second; // 这里是vScw的第一次赋值，vScw就是闭环中矫正过的Sim3位姿
    ...
    // LoopConnections里面是<CurrentFrame的一级共视帧，set里面装的是一级共视帧的共视帧（即二级共视帧）>，我记得是一级共视帧才用回环更新了Sim3的
    for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend=LoopConnections.end(); mit!=mend; mit++)
    {
        ...
        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            ...
            /* 采自 types_seven_dof_expmap.h 文件：
             *      Sim3 C(_measurement); //Measurement是Sji i到j的变换
             *      Sim3 error_=C*v1->estimate()*v2->estimate().inverse();
             *      _error = error_.log();
             *  v1是nIDi，v1*v2.inverse就是Tiw*Twj，C*Tiw*Twj如果没有error，结果应该是单位矩阵
             *  单位矩阵求.log()得到的就是0，故没有error得到的_error也是0
             *  顶点优先取回环纠正后的Sim3来优化，次要取未纠正的->GetRotation和->GetTranslation得到的R,t优化
             */
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        LoopClosing::KeyFrameAndPose::const_iterator iti = NonCorrectedSim3.find(pKF); // NonCorrectedSim3里面存的是没有经过回环矫正的Sim3
