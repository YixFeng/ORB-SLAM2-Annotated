                                       ...
                                       vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo)
{
    ...
    while(f1it!=f1end && f2it!=f2end)
    {
        ...
        {
            f1it = vFeatVec1.lower_bound(f2it->first); // lower_bound取的是在这个vec里大于等于()里这个数的那一位
        }
        else
        {
            f2it = vFeatVec2.lower_bound(f1it->first);
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        // 点距离越远，希望的搜索半径就越小，这里MAXDISTENSE对应的Level是0层，故离最远的时候Scale不用扩大
        // 点的距离对应的Level是用换底公式算的Level = log((maxdist/currentdist) / 1.2) = lg(maxdist/currentdist)/lg(1.2)
...
// 每行是一个特征点的描述子cols = 32，CV_8U，Mat形式，每个元素格式都是Unsigned Char的8位字符
