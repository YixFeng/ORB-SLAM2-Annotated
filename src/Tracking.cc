...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        // i是上一帧有深度的点的ID，如果mvpMapPoints对应的ID没有地图点，就创建一个临时地图点
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // mVelocity 上一个Frame模型计算出的上一帧和上上帧之间的位姿变换 i.e. 恒速模型，用之前的变换初始化当前的
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    int nRefMatches = mpReferenceKF->TrackedMapPoints(nMinObs); // 计数：被观测次数多于3次的地图点
        ...
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                if(nGood<50)
                {
                    ...
                    // 取的是参考帧中未用BoW匹配的地图点，用PnP投影在当前帧的图片中，通过和<恒速模型>中相同的方式：取半径找半径内最匹配的特征点，构成点匹配
                        ...
                        if(nGood>30 && nGood<50)
                        {
                            ...
                            for(int ip =0; ip<mCurrentFrame.N; ip++)
                                if(mCurrentFrame.mvpMapPoints[ip])
                                    sFound.insert(mCurrentFrame.mvpMapPoints[ip]); // 把nGood<50的PnP加进去的匹配也都算进sFound已找到的匹配中
